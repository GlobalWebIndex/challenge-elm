...
{-| Determine if any tree satisfy some test.

This function is mutually recursing on the `isInForest` function.

This function usually is the entry point of the mutual recursion,
and specifies the behavior on the tree.
...
-}
isInTree : (a -> Bool) -> Tree a -> Bool
isInTree predicate (Tree value forest) =
    predicate value || isInForest predicate forest


{-| Determine if any forest satisfy some test.

This function is mutually recursing on the `isInTree` function.

This function specifies the behavior on the forest.

Here is a pseudocode for better understanding:

        isInForest : (a -> Bool) -> Forest a -> Bool
        isInForest predicate forest =
            case forest of
                [] ->
                    False

                x :: xs ->
                    isInTree predicate x || isInForest predicate xs
...
isInForest : (a -> Bool) -> Forest a -> Bool
isInForest predicate forest =
    if LL.isEmpty forest then
        False

    else
        (forest
            |> LL.head
            |> Maybe.map (isInTree predicate)
            |> Maybe.withDefault False
        )
            || (forest
                    |> LL.tail
                    |> Maybe.map (isInForest predicate)
                    |> Maybe.withDefault False
               )
